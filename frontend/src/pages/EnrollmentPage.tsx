...
export function EnrollmentPage() {
  ...
  const canSubmit = useMemo(
    () => speakerName.trim().length > 0 && files.length > 0,
    [speakerName, files],
  );
  ...
  const submit = async () => {
    ...
    try {
      ...
      const message = error?.response?.data?.detail || error?.response?.data?.message || "Enrollment failed";
    ...
    <PageShell>
      <div className="p-6">
        ...
        <SectionTitle
          ...
          title="Enrollment"
          subtitle="Upload or record multiple samples, preview the waveform, and save a new speaker profile."
        />
        <div className="mt-8 grid gap-6 xl:grid-cols-[0.85fr_1.15fr]">
          <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
            <input
              ...
              className="w-full rounded-2xl border border-white/10 bg-slate-950/40 px-4 py-3 outline-none"
            />
          ...
          <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              {["Audio preview", "File validation", "Progress tracking"].map(
                (item) => (
                  <div
                    key={item}
